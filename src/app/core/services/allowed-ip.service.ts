...
export class AllowedIpService {
  private readonly url = `${environment.apiUrl}/api/users/allowed-ips`;

  constructor(private http: HttpClient) {}

  getAllowedIps(username: string): Observable<ApiResponse<string[]>> {
    const params = new HttpParams().set('username', username);
    return this.http.get<ApiResponse<string[]>>(this.url, { params });
  }

  addAllowedIp(username: string, ipAddress: string): Observable<ApiResponse<string[]>> {
    return this.http.post<ApiResponse<string[]>>(this.url, { username, ipAddress });
  }

  removeAllowedIp(username: string, ipAddress: string): Observable<ApiResponse<string[]>> {
